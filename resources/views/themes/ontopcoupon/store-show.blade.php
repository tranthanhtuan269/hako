...
<div class="otc-store-page">
    <div class="container">
        <div class="otc-store-hero">
            @include('partials.store-logo', ['store' => $store, 'size' => 'xl', 'showVerified' => false, 'linked' => false])
            <div class="otc-store-hero-body">
                <p class="otc-kicker">Verified store</p>
                <h1>{{ $store->name }} Promo Codes &amp; Coupons</h1>
                @if($store->description)
                    <p class="otc-store-hero-desc">{{ \Illuminate\Support\Str::limit(strip_tags($store->description), 220) }}</p>
                @else
                    <p class="otc-store-hero-desc">Browse verified {{ $store->name }} coupon codes and discount deals updated on {{ config('site.domain') }}.</p>
                @endif
        ...
        <div class="otc-store-layout">
            <div class="otc-store-main">
                ...
                <div class="otc-coupon-list">
                    ...
                    @empty
                        <p class="otc-empty">No coupons available for this store yet.</p>
                    @endforelse
                </div>
            ...
            <aside class="otc-store-sidebar">
                @if($similarStores->isNotEmpty())
                    <div class="otc-sidebar-box">
                        <h3>Similar Stores</h3>
                        <ul class="otc-similar-stores">
                ...
                @if($topCategories->isNotEmpty())
                    <div class="otc-sidebar-box">
                        ...
                        <div class="otc-category-tags">
                            @foreach($topCategories as $category)
                                <a href="{{ route('categories.show', $category->slug) }}" class="otc-category-tag">{{ $category->name }}</a>
                            @endforeach
                        </div>
                    </div>
                @endif
