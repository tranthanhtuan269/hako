...
APP_DIR="/var/www/buyreviewwise"
DB_NAME="buyreviewwise"
DB_USER="buyreviewwise"
DB_PASS="${DB_PASS:?DB_PASS required}"
DOMAIN="buyreviewwise.com"
GIT_BRANCH="coupontracer"
GIT_REPO="https://github.com/tranthanhtuan269/hako.git"
ADMIN_EMAIL="admin@buyreviewwise.com"
ADMIN_PASS="buyreviewwise.com@"
SCAN_SITE="buyreviewwise"
...
NGINX_SITE="/etc/nginx/sites-available/buyreviewwise.com"
...
ln -sf "${NGINX_SITE}" /etc/nginx/sites-enabled/buyreviewwise.com
rm -f /etc/nginx/sites-enabled/default

nginx -t
systemctl reload nginx

mkdir -p /etc/buyreviewwise
cat > /etc/buyreviewwise/db.env <<EOF
DB_DATABASE=${DB_NAME}
DB_USERNAME=${DB_USER}
DB_PASSWORD=${DB_PASS}
EOF
cat > /etc/buyreviewwise/deploy.env <<EOF
SITE_NAME=buyreviewwise
APP_DIR=${APP_DIR}
GIT_REPO=${GIT_REPO}
GIT_BRANCH=${GIT_BRANCH}
LOG_FILE=/var/log/buyreviewwise-auto-update.log
LOCK_FILE=/var/run/buyreviewwise-auto-update.lock
GIT_CREDENTIALS_FILE=/etc/buyreviewwise/git-credentials
EOF
