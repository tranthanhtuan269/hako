...
APP_DIR="/var/www/hopereview"
DB_NAME="hopereview"
DB_USER="hopereview"
DB_PASS="${DB_PASS:?DB_PASS required}"
DOMAIN="hopereview.com"
GIT_BRANCH="ontopcoupon"
GIT_REPO="https://github.com/tranthanhtuan269/hako.git"
ADMIN_EMAIL="admin@hopereview.com"
ADMIN_PASS="hopereview.com@"
SCAN_SITE="hopereview"
THEME_KEY="ontopcoupon"
...
\App\Support\ThemeManager::set('${THEME_KEY}');
"
...
NGINX_SITE="/etc/nginx/sites-available/hopereview.com"
...
ln -sf "${NGINX_SITE}" /etc/nginx/sites-enabled/hopereview.com
rm -f /etc/nginx/sites-enabled/default

nginx -t
systemctl reload nginx

mkdir -p /etc/hopereview
cat > /etc/hopereview/db.env <<EOF
DB_DATABASE=${DB_NAME}
DB_USERNAME=${DB_USER}
DB_PASSWORD=${DB_PASS}
EOF
cat > /etc/hopereview/deploy.env <<EOF
SITE_NAME=hopereview
APP_DIR=${APP_DIR}
GIT_REPO=${GIT_REPO}
GIT_BRANCH=${GIT_BRANCH}
LOG_FILE=/var/log/hopereview-auto-update.log
LOCK_FILE=/var/run/hopereview-auto-update.lock
GIT_CREDENTIALS_FILE=/etc/hopereview/git-credentials
EOF
